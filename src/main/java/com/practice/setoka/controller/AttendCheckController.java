...
public class AttendCheckController {
    ...
    @PostMapping("/point")
    public String attendPoint(@RequestParam(name = "date", required = false) String date, Model model) {
        LocalDate today = LocalDate.now();
        LocalDate clickedDate = LocalDate.parse(date);
        
    	if(!clickedDate.equals(today)) {
    		model.addAttribute("error", "다른 날은 출석할 수 없습니다.");
    	}
    	attendCheckService.updatePoint(.getUserNum());
