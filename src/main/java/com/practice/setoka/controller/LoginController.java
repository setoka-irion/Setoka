...
public class LoginController {
	...
	@ResponseBody
	public ResponseEntity<String> SingupSubmit(@RequestBody @Valid UsersDto dto,
            @RequestParam("passwordCom") String passwordCom,
            HttpSession session) {
	    ...
	    if (!userService.passwordInvalid(dto.getPassword())) {
	        return ResponseEntity.badRequest().body("대소문자 특수문자 포함 8자 이상");
	    }

	    if (!dto.getPassword().equals(passwordCom)) {
	        return ResponseEntity.badRequest().body("비밀번호 확인이 일치하지 않음");
	    }

	    if (userService.existsByName(dto.getId())) {
	        return ResponseEntity.badRequest().body("중복 아이디");
	    }

	    if (dto.getNickName().isEmpty()) {
	        return ResponseEntity.badRequest().body("닉네임이 비어있음");
	    }

	    if (dto.getRealName().isEmpty()) {
	        return ResponseEntity.badRequest().body("이름이 비어있음");
	    }

	    if (dto.getPhoneNumber().isEmpty()) {
	        return ResponseEntity.badRequest().body("전화번호가 비어있음");
	    }

	    Users ni = userService.selectByNickName(dto.getNickName());
	    if (ni != null && ni.getStatus() == Status.정상) {
